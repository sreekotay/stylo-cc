...
**The gate.** The two printouts must be byte-identical before any time is recorded. A row is one element (or `::before` / `::after` box) and all 189 CSS longhands Stylo exposes to `getComputedStyle`, in Stylo's order and Stylo's serialization — not just the properties the benchmark sheet happens to set. Every suite, every size, plus a set of hand-written fixtures (cascade order, `!important`, `em` / `rem` / `%`, `calc()`, `currentcolor`, `style=` attributes, inherited changes reaching untouched descendants) passes `cmp` clean.

**Completeness.** The property list is Stylo's own (`stylo-runner --longhands`), turned into code at compile time by a `@comptime` generator, so every element carries the full computed-style width: each of Stylo's 20 style structs is inherited or reset, every length resolved, every value printed. Lengths accept `calc()` over `px` / `em` / `rem` / `%` / numbers (`fixtures/local/calc.stylebench`). `:is()` / `:not()` / `:where()` take a selector list (`fixtures/local/is-not.stylebench`). `var()` substitutes custom properties (`fixtures/local/var.stylebench`). Box shorthands assign 1–4 side tokens (or a 1–2 pair) onto those longhands (`fixtures/local/box.stylebench`). `:hover` is the interned atom `":hover"` on the node's class list; `+hover N` writes it on N and its ancestors (`fixtures/local/hover.stylebench`). What the engine does not do is the rest of CSS — `min`/`max`/`clamp`, value lists (`font-family` stacks, `transform` functions), `:active` / `:focus` and other user-action, layout-time semantics. On StyleBench none of that is exercised, so the output is the same; on arbitrary CSS it would not be.
...
| `calc.stylebench` | `calc()` on lengths: fold, `*` / `/`, nested, `em`/`rem`, leftover `%` ± px, `!important`, `style=`, inherit, class add/remove re-resolve, invalid drop |
| `is-not.stylebench` | `:is` / `:not` / `:where`: OR list, nested `:not`, combinator inside `:is`, `:where` specificity 0, ancestor `:not` class mut, empty list drop |
| `var.stylebench` | `var()` / `--*`: substitute, fallback, inherit, `calc(var())`, missing / cycle drop, `style=`, ancestor class mut |
| `box.stylebench` | 1–4 side tokens (`margin` / `padding` / `border-width` / `border-style` / `border-color` / `inset`), `overflow` / `gap` pairs, `border` / `border-top`, leftover drop, `calc` / `var` components, `style=`, class mut |
| `hover.stylebench` | `:hover` as the class atom `":hover"`: `+hover N` writes N and ancestors, subject, descendant, `+` / `~`, `:not` / `:is`, share twins |
...
Value cells (`engine/cssval.cch`, plain C so the `@comptime` emitter and the runtime share them): an **atom** (interned keyword, or the text kept verbatim), a **length** (number + `px` / `em` / `rem` / `%`, or `auto` / `normal` / `none` / …; specified is a linear combination of those four bases so `calc(2em + 4px)` re-resolves on inherit), a **color** (`rgb()` / `rgba()` / `#rgb` … `#rrggbbaa` / `transparent` / `currentcolor`). `font-size` `%` and `em` resolve against the parent, `rem` against the root; mixed `calc()` folds `em`/`rem` into `px` and keeps leftover `%` for dump (`calc(50% + 8px)`); `currentcolor` on `color` is the inherited color.
...
Cascade: UA / author / `style=` origin, `!important` (author important < style important < UA important), specificity, source order; `background` and the box shorthands (`margin` / `padding` / `border-*` / `inset` / `overflow` / `gap`) expand onto their longhands; CSS-wide `initial`.

Selectors: type, `#id`, `.class`, `[attr]` / `[attr=val]`, `*`, descendant, child, next-sibling (`+`), subsequent-sibling (`~`), `:first-child` / `:last-child` / `:first-of-type` / `:last-of-type` / `:only-of-type` / `:empty`, `:nth-child(an+b)` / `:nth-last-child(an+b)` / `:nth-of-type(an+b)` / `:nth-last-of-type(an+b)` (`odd` / `even` too; no whitespace inside the parens, no `of S`), `:is()` / `:not()` / `:where()` (selector list, combinators and nesting; not a forgiving list — one bad alt drops the rule; no quoted `)` in an arg), `:hover` (the interned atom `":hover"` on the class list), `::before` / `::after` on the subject. At-rules: `@media (min-width | max-width: Npx)` blocks, evaluated against the viewport width (800 px until a `resize` step).

**Not in this engine.** The rest of CSS. In particular:

- **Selectors:** `:only-child`, `:nth-*(… of S)`, `:has`, `:active` / `:focus` and other user-action besides `:hover`, `::first-line` / `::first-letter` and other pseudo-elements, media features beyond width, `@supports`, shadow, namespaces.
- **Values:** `calc()` with `min`/`max`/`clamp` / colors, hsl, named colors beyond `black` / `white` / `red` / `transparent` / `currentColor`, multi-value lists (`font-family` stacks, `transform` functions), `font` / logical / grid shorthands. Custom properties are inherited token text; `var()` / `var(--n, fb)` substitutes before the cell parser (including inside `calc()`). A declaration the cell parser rejects is dropped, as an invalid declaration would be — so the width is Stylo's, the value grammar is still not CSS.
...
- Stylo parses and computes the full value grammar (`var()`, lists, every keyword, `min`/`max`/`clamp`). We parse what StyleBench and the local fixtures write, including length `calc()` over `px` / `em` / `rem` / `%` / numbers, `var()` over inherited `--*`, and the box shorthands — a value we cannot parse is dropped as invalid. On these fixtures that is the same computed style; on arbitrary CSS it would not be.
