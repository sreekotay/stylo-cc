...
The CC runner is a styling engine (tree, match, cascade, computed values, walk) written the [CC way](https://github.com/sreekotay/concurrent-c/blob/main/docs/the-cc-way.md). Sequential first; parallel once the relations are nameable.

Same print format. `cmp` is the gate. Timings come after receipts match.

Tiny suite receipt: 81/81 background-colors match
(`receipts/tiny_2026_08_29.txt`). Fixture comments are `# ` only — a leading
`#ident` is a CSS id selector.
