...
**Plug-in seam.** `LIBWEB_STYLE_ENGINE=cc` links `libstylecc.a` behind `StyleEngineBoundary.json`: the Concurrent-C engine mirrors apply transactions; Rust still owns take/planning until `STYLECC_OWN_TAKE=1` (opt-in; needs rule translation). Build: `LIBWEB_STYLE_ENGINE=cc scripts/browser-bench.sh ladybird-build`. Receipt (mirror path, Descendant/child, conservative + internals): `receipts/browser-ladybird-stylecc.txt` — sync mean ~216 ms / style ~107 ms / engine ~93 ms over 5 iters on 20k live.
