...
**Plug-in seam.** The `ladybird/` submodule stays pinned. Our seam is `patches/ladybird/` (`scripts/ladybird-patches.sh`); `ladybird-build` applies it. `LIBWEB_STYLE_ENGINE=cc` links `libstylecc.a` (`-DLIBWEB_STYLE_ENGINE_CC=ON`). Transactions and StyleBench-shaped rules update both engines. Default take is Ladybird rematch (`STYLECC_FORWARD=1`). `STYLECC_OWN_TAKE=1` lets CC match and cascade: the host copies used style onto interned Ladybird groups and Installs a style-record identity. Layout is Ladybird's. Receipts: `receipts/browser-ladybird-stylecc-forward.txt` (rematch) and `receipts/browser-ladybird-stylecc-own.txt` (CC). See README “CC matching in Ladybird”.
